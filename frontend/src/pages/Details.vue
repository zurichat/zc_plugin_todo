    ...
    <div class="td-w-full td-flex-grow ">
        <div id="Details" class="px-2">
            <div class="todo-nav td-bg-green-500 td-flex td-items-center">
                <h1 class="td-px-4 td-py-2 td-text-white td-text-3xl">
                    # To-do-list
                </h1>

                <span class="btn"
                    ><i class="pi td-td- td-px-4 pi-chevron-down"
                /></span>
            </div>
            <div
                class="header td-py-4 td-flex td-justify-between td-items-center td-border-b-2"
            >
                <div>
                ...
                <div class="td-flex td-flex-row td-items-center">
                    ...
                    <div class="td-relative" v-click-away="ClickAway">
                        <span
                            @click="assign()"
                            class="td-bg-green-500 td-h-10 td-w-10 td-mx-2 td-justify-center td-flex td-items-center td-rounded-xl"
                            ><i
                                class="pi td-text-white pi-user-plus td-cursor-pointer"
                        /></span>
                        <div
                            v-if="isAssign"
            ...
            <div
                class="sub-header td-flex td-py-3 td-justify-between td-items-center td-border-b-2"
            >
                <div class="td-flex">
                    <div
                        class="mr-2 font-bold button td-px-4 td-py-2 td-bg-green-500 td-cursor-pointer td-rounded td-text-white"
                    >
                        + Add a new Task
                    </div>
                    <div
                        class="amt_completed td-px-4 td-py-2 td-flex td-items-center"
                    >
                        <svg
                            width="19"
                            height="19"
      ...
      <span>Our todo plugin is a collection which means more than one todo can be made. Each todo can contain multiple tasks containing which includes: the title, the description, list of check able task. Note each todo is a room, when you create a todo you’ve automatically created a channel with the  same name (visible in the side bar) and everyone assigned to it will be in that channel. At the point of creating the todo you can decide to make it private or public </span>
            ...
            <div class="tasks_container td-py-4" v-if="getTasks.length">
                    ...
                    <div class="tabContents">
                        <div v-if="isActive === '1'">
                            <div
                                ...
                                :key="index"
                                class="td-pl-4 td-m-4 task_box td-flex td-py-2 hover:shadow td-rounded hover:border"
                            >
                                <Checkbox
                                    v-model="checked"
                                    :id="'city' + index"
                                ...
                                <div class="task_content td-flex-grow td-pl-2">
                                    <div class="td-flex td-justify-between">
                                        <p
                                            class="task_title td-pb-3 td-font-medium"
                                        >
                                            <span class="task_number"
                                                >Task
                                                {{
                                                    taskSerialNumber(index + 1)
                                                }}
                                                -</span
                                            >
                                            {{ task.title }}
                                        ...
                                        <p>
                                            <i
                                                class="pi pi-ellipsis-v td-cursor-pointer"
                                            ></i>
                                        </p>
                                    </div>

                                    <div
                                        class="task_details td-flex td-flex-row td-justify-between"
                                    >
                                        <div
                                            class="task_comment-amt td-flex td-items-center"
                                        >
                                            <div
                                                class="todo-profileImg2 td-flex"
                                            >
                                                <div
                                                    class="todo-profile profileOne"
                                                >
                                                    <img
                                                        src="../assets/img/collaborators.svg"
                                                        style="border: 1px solid rgb(1, 216, 146); border-radius: 4px;"
                                        ...
                                        <div
                                            class="task_tag td-flex td-flex-row td-items-center"
                                        >
                                            <div
                                                class="td-text-gray-500 td-pr-1"
                                            >
                                                <i class="pi pi-tag"></i
                                                ><span
                                            ...
                                            <div
                                                class="td-text-blue-500 td-pr-1"
                                            >
                                                <i class="pi pi-calendar"></i
                                                ><span
                                                    class="td-px-2 td-font-bold td-text-sm"
                                                    >Tomorrow</span
                                                >
                                            </div>
                                            <div></div>
...
export default {
    ...
    computed: {
        ...
        getTasksPayload() {
            return {
                todo_id: "614215409fd1f4f655d44594",
                user_id: "6139a43559842c7444fb01ef",
                org_id: "613a3ac959842c7444fb0240"
            };
        },
    ...
    methods: {
        ...mapActions({ getAllTasks: "task/fetchTodoTasks" }),

        taskSerialNumber(index) {
            return index < 9 ? `0${index}` : index;
        },

        getTodoTasks() {
            console.log("calling", this.getTasksPayload);
            this.getAllTasks(this.getTasksPayload)
                .then(res => {
                    console.log("todo tasks", res);
                })
                .catch(err => {
                    console.log("error getting tassk", err);
                });
        },
        ...
        check() {
            ...
            this.selectedTodo = this.allTodos.find(
                todo => todo.card_id.toLowerCase() === id.toLowerCase()
            );
